...
const ClientDetailView: React.FC = () => {
    ...
    const { clients, staff, saveClient, deleteClient, serviceApplications, tickets, invoices, hasPermission, plans, seminars, events, auditLogs, clientUsers, getStaffDisplayName, approvePlanChange } = useClientData();
    ...
    const handleApprovePlanChange = () => {
        if (client.pendingPlanChange && window.confirm(`${client.companyName}のプラン変更を承認しますか？`)) {
            approvePlanChange(client.id);
            alert('プラン変更を承認しました。');
        }
    };
    ...
    return (
        <div className="fade-in space-y-6">
            ...
            {client.pendingPlanChange && (
                 <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 p-4 rounded-md" role="alert">
                    <div className="flex justify-between items-center">
                        <div>
                            <p className="font-bold">プラン変更申請あり</p>
                            <p className="text-sm">
                                新プラン「{plans.find(p => p.id === client.pendingPlanChange?.planId)?.name}」への変更が申請されています。
                                適用予定日: {new Date(client.pendingPlanChange.effectiveDate).toLocaleDateString()}
                            </p>
                        </div>
                        <button onClick={handleApprovePlanChange} className="bg-yellow-500 text-white px-4 py-2 rounded-md hover:bg-yellow-600">
