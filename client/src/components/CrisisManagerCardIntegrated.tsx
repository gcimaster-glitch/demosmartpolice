...
interface CrisisManagerCardIntegratedProps {
    hasDedicatedManager: boolean;
    crisisManager: CrisisManager | null;
}

const CrisisManagerCardIntegrated: React.FC<CrisisManagerCardIntegratedProps> = ({ 
    hasDedicatedManager, 
    crisisManager 
}) => {
    ...
    const { name, realName, position, phone, email, photoUrl, profile } = crisisManager;
    const displayName = name || realName;
    const defaultPhoto = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(displayName) + '&background=3B82F6&color=fff&size=128';
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 border-l-4 border-blue-500 border">
                ...
                <div className="flex items-center space-x-4 mb-5">
                    ...
                    <div className="flex-1">
                        ...
                        {position && <p className="text-sm text-blue-600 font-medium">{position}</p>}
                        <p className="text-xs text-gray-500">あなた専属の担当者</p>
                    </div>
                </div>

                {profile && <p className="text-xs text-gray-600 bg-gray-50 p-3 rounded-md mb-4 border">{profile}</p>}
...
export default CrisisManagerCardIntegrated;
