...
const Seminars: React.FC = () => {
    const { seminars, applyForSeminar, getSeminarApplicationStatus, currentClient } = useClientData();
    const { user } = useAuth();
    const [categoryFilter, setCategoryFilter] = useState<CategoryFilter>('すべて');
    const [viewMode, setViewMode] = useState<ViewMode>('card');
    const [selectedSeminar, setSelectedSeminar] = useState<Seminar | null>(seminars[0] || null);
    const [mainImage, setMainImage] = useState<string | null>(seminars[0]?.mainImageUrl || null);
    ...
    const handleApplySubmit = (data: { notes: string; userName: string; userEmail: string; }) => {
        if (!user || !currentClient || !selectedSeminar) {
            alert('ユーザー情報またはセミナー情報が見つかりません。');
            return;
        }
        
        const applicationData: Omit<SeminarApplication, 'seminarId' | 'applicationDate'> = {
        ...
        const result = applyForSeminar(applicationData, selectedSeminar.id);
        alert(result.message);

        if (result.success) {
            // Optimistic update for UI
            const newApplicationForState: SeminarApplication = {
                ...applicationData,
                seminarId: selectedSeminar.id,
                applicationDate: new Date().toISOString(),
            };
            setSelectedSeminar(prev => prev ? { ...prev, applicants: [...prev.applicants, newApplicationForState] } : null);
                ...
                <aside className="xl:w-3/5">
                    {viewMode === 'card' ? (
                        ...
                        <div className="bg-white rounded-lg shadow-md overflow-hidden">
                            ...
                            </tr></thead><tbody className="bg-white divide-y divide-gray-200">{filteredSeminars.map(s => <SeminarListItem key={s.id} seminar={s} />)}</tbody></table>
                        </div>
                    )}
                </aside>
